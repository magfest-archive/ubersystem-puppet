...
subj_organization=$5
subj_common_name=$6
subj="/C=$subj_country_code/ST=$subj_state/L=$subj_location/O=$subj_organization/CN=$subj_common_name"
...
openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "$subj" -keyout $ssl_ca_key -out $ssl_ca_crt

# create server key and signing request
openssl genrsa -out $ssl_server_key 1024
openssl req -new -nodes -subj "$subj" -key $ssl_server_key -out $ssl_server_csr
...
openssl req -new -nodes -subj "$subj" -key $ssl_client_key -out $ssl_client_csr
